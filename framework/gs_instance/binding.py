...
from sld import make_sld, get_rgb_json
import getpass
...
def main(cat, folder, sld_folder, pycsw_url):
    ...
    for raster in rasters:
        ...
        if cat.get_resource(name):
            print 'The layer {name} is already integrated'.format(name=name)
        else:
            dirs = os.path.dirname(base).split('/')
            country_wetland = dirs[-2]
            print country_wetland
            ...
            cat.create_coveragestore(name, raster, ws)
            store = cat.get_resource(name)
            product = 'SWD'
            layer = cat.get_layer(name)
            layer._set_default_style(product)
    '''
